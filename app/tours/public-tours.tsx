...
export function PublicTours({ staffPreview = false }: { staffPreview?: boolean }) {
  const { t } = useI18n();
  ...
  useEffect(() => {
    fetch(staffPreview ? "/api/tours/public?preview=1" : "/api/tours/public", { cache: "no-store" })
      .then(async (response) => {
  ...
  if (!settings.publicEnabled && !staffPreview) {
    return (
  ...
  return (
    <div className="space-y-6">
      {staffPreview && !settings.publicEnabled ? (
        <div className="rounded-xl border border-indigo-200 bg-indigo-50 px-4 py-3 text-sm text-indigo-800">
          {t("tours.public.staffPreviewNotice")}
        </div>
      ) : null}
      {tours.length === 0 ? (
          ...
          {tours.map((tour) => {
            ...
            return (
              <article key={tour.id} className="flex flex-col rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                <div className="mb-2 self-start rounded-md bg-indigo-100 px-2 py-1 text-xs font-bold text-indigo-800">{t("tours.number", { number: tour.tourNumber })}</div>
